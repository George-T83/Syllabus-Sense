...
describe('applySM2', () => {
  it('sets a 1-day interval on the first "good" review', () => {
    const result = applySM2(SM2_DEFAULTS, 'good', new Date(2026, 8, 3));
  ...
  it('sets a 6-day interval on the second consecutive "good" review', () => {
    const first = applySM2(SM2_DEFAULTS, 'good', new Date(2026, 8, 3));
    const second = applySM2(first, 'good', new Date(2026, 8, 4));
  ...
  it('multiplies the interval by the ease factor from the third review onward', () => {
    const first = applySM2(SM2_DEFAULTS, 'good', new Date(2026, 8, 3));
    const second = applySM2(first, 'good', new Date(2026, 8, 4));
    const third = applySM2(second, 'good', new Date(2026, 8, 10));
  ...
  it('resets repetitions and interval to 1 day on "again" (a lapse)', () => {
    const first = applySM2(SM2_DEFAULTS, 'good', new Date(2026, 8, 3));
    const second = applySM2(first, 'good', new Date(2026, 8, 4));
    const lapsed = applySM2(second, 'again', new Date(2026, 8, 10));
  ...
  it('treats "hard" as a lapse too (quality below the SM-2 pass threshold)', () => {
    const result = applySM2(SM2_DEFAULTS, 'hard', new Date(2026, 8, 3));
    expect(result.repetitions).toBe(0);
    expect(result.interval).toBe(1);
  });

  it('increases the ease factor on "easy" reviews', () => {
    const result = applySM2(SM2_DEFAULTS, 'easy', new Date(2026, 8, 3));
  ...
  it('never lets the ease factor drop below 1.3, even after repeated lapses', () => {
    ...
    for (let i = 0; i < 5; i++) {
      state = applySM2(state, 'again', new Date(2026, 8, 3));
    }
...
describe('isCardDue', () => {
  it('is due when the due date is today', () => {
    expect(isCardDue({ dueDate: '2026-09-03' }, new Date(2026, 8, 3))).toBe(true);
  });

  it('is due when the due date is in the past', () => {
    expect(isCardDue({ dueDate: '2026-09-01' }, new Date(2026, 8, 3))).toBe(true);
  });

  it('is not due when the due date is in the future', () => {
    expect(isCardDue({ dueDate: '2026-09-10' }, new Date(2026, 8, 3))).toBe(false);
  });
});
