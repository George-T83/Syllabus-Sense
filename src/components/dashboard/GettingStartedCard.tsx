...
function PrimaryAction({ children, onClick }: { children: ReactNode; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="inline-flex min-h-[44px] items-center gap-1.5 rounded-full bg-[#5b3df5] px-4 text-xs font-semibold text-white shadow-sm transition-transform hover:scale-[1.02] active:scale-[0.97]"
    >
      <svg
        className="h-4 w-4"
        fill="none"
  ...
  const actionsFor = (id: SetupStepId): ReactNode => {
    switch (id) {
      case 'course':
        return (
          <>
            <PrimaryAction onClick={onUploadSyllabus}>Upload a syllabus</PrimaryAction>
